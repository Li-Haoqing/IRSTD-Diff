...
from .losses import normal_kl, discretized_gaussian_log_likelihood, SoftIoU, criterion
from scipy import ndimage
from torchvision import transforms
from utils.utils import norm, iou
...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        ...
        x = x[:, -1:, ...]  # loss is only calculated on the last channel
    ...
    def p_sample_loop_known(
        ...
        if self.dpm_solver:
            ...
            if iou(final["sample"][:, -1, :, :].unsqueeze(1), final["cal"]) < 0.65:
                cal_out = torch.clamp(final["cal"] + 0.25 * final["sample"][:, -1, :, :].unsqueeze(1), 0, 1)
            else:
    ...
    def p_sample_loop_progressive(
        ...
        if progress:
            ...
            for i in indices:
                ...
                with th.no_grad():
                    if img.size(1) != org_c:
                        img = torch.cat((org_img, img), dim=1)
    ...
    def training_losses_segmentation(self, model, classifier, x_start, t, model_kwargs=None, noise=None):
        ...
        mask = x_start[:, -1:, ...]
        res = torch.where(mask > 0, 1, 0)  # mask

        res_t = self.q_sample(res, t, noise=noise)  # add noise
        x_t = x_start.float()
        x_t[:, -1:, ...] = res_t.float()
        terms = {}
