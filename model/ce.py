...
from guided_diffusion.nn import normalization
from .liw import LIW
...
class IPOF(nn.Module):
    ...
    def __init__(self, in_channel, kernel_size=1, stride=1):
        ...
        self.conv_1 = nn.Sequential(nn.Conv2d(self.in_channel, self.inter_channel, kernel_size=1, stride=stride, padding=0, bias=False),
                                    nn.BatchNorm2d(self.inter_channel),
                                    nn.ReLU(True),
                                    nn.Conv2d(self.inter_channel, 1, kernel_size=1, stride=stride, padding=0, bias=False),
                                    nn.BatchNorm2d(1),
                                    nn.ReLU(True))
        self.conv_2 = nn.Sequential(nn.Conv2d(self.in_channel, self.inter_channel, kernel_size=1, stride=stride, padding=0, bias=False),
                                    nn.BatchNorm2d(self.inter_channel),
                                    nn.ReLU(True),
        ...
        self.conv_3 = nn.Sequential(nn.Conv2d(self.in_channel, self.inter_channel, kernel_size=1, stride=stride, padding=0, bias=False),
                                    nn.BatchNorm2d(self.inter_channel),
                                    nn.ReLU(True),
                                    nn.Conv2d(self.inter_channel, self.inter_channel, kernel_size=1, stride=stride, padding=0, bias=False),
                                    nn.BatchNorm2d(self.inter_channel),
        ...
        self.conv_4 = nn.Sequential(nn.Conv2d(self.in_channel, self.inter_channel, kernel_size=1, stride=stride, padding=0, bias=False),
                                    nn.BatchNorm2d(self.inter_channel),
                                    nn.ReLU(True),
                                    nn.Conv2d(self.inter_channel, self.inter_channel, kernel_size=1, stride=stride, padding=0, bias=False),
                                    nn.BatchNorm2d(self.inter_channel),
...
class CE(nn.Module):
    ...
    def __init__(self, cfg: dict, out_ch: int = 1):
        ...
        self.bn = nn.BatchNorm2d(out_ch)
        self.liw = LIW(128, in_channel=64, middle_channel=64, level=2)
    ...
    def forward(self, x: torch.Tensor):
        ...
        x = encode_outputs.pop()
        decode_outputs = [x]
        for i, m in enumerate(zip(self.decode_modules, self.ipof_modules)):
            x2 = encode_outputs.pop()
            if i == 3:
                x2 = self.liw(x2)  # [N, 64, 128, 128]
                # x2 = torch.cat([x2, w], dim=1)
            x = F.interpolate(x, size=x2.shape[2:], mode='bilinear', align_corners=False)
            x = m[1](x, x2)
            x = m[0](x)
            decode_outputs.insert(0, x)
        decode_embs = decode_outputs.copy()
        ...
        if self.training:
            return decode_embs, [x] + side_outputs
        else:
            return decode_embs, self.bn(x)
            # return decode_embs, x


def CE_l(out_ch: int = 1):
    cfg = {
    ...
    return CE(cfg, out_ch)


def CE_m(out_ch: int = 1):
    cfg = {
        "encode": [[7, 3, 16, 64, False, False],  # En1
                   [6, 64, 16, 64, False, False],  # En2
                   [5, 64, 32, 128, False, False],  # En3
                   [4, 128, 32, 256, False, False],  # En4
                   [4, 256, 32, 256, True, False],  # En5
                   [4, 256, 64, 256, True, True]],  # En6
        # height, in_ch, mid_ch, out_ch, RSU4F, side
        "decode": [[4, 512, 64, 256, True, True],  # De5
                   [4, 512, 32, 128, False, True],  # De4
